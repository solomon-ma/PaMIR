...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        self.pamir_multiview_net = PamirNetMultiview().to(self.device)

        # optimizers
        # self.optm_pamir_net = torch.optim.RMSprop(
        #     params=list(self.pamir_net.parameters()), lr=float(self.options.lr)
        # )
        self.optm_pamir_multiview_net = torch.optim.RMSprop(
            params=list(self.pamir_multiview_net.parameters()), lr=float(self.options.lr)
        )
        # loses
        ...
        self.models_dict = {'pamir_multiview_net': self.pamir_multiview_net}
        self.optimizers_dict = {'optimizer_pamir_multiview_net': self.optm_pamir_multiview_net}
        ...
        # if self.options.pretrained_pamir_multiview_net_checkpoint is not None:
        #     self.load_pretrained_pamir_multiview_net(self.options.pretrained_pamir_multiview_net_checkpoint)
        ...
        logging.info('#trainable_params = %d' %
                     sum(p.numel() for p in self.pamir_multiview_net.parameters() if p.requires_grad))
    ...
    def train_step(self, input_batch):
        self.pamir_multiview_net.train()
        ...
        output_sdf = self.pamir_multiview_net(img, vol, pts, pts_proj)
        ...
        # Do backprop
        self.optm_pamir_multiview_net.zero_grad()
        total_loss.backward()
        self.optm_pamir_multiview_net.step()
        ...
        if self.step_count % 10000 == 0:
            ...
            for param_group in self.optm_pamir_multiview_net.param_groups:
    ...
    def load_pretrained_pamir_multiview_net(self, model_path):
        if os.path.isdir(model_path):
            tmp1 = glob.glob(os.path.join(model_path, 'pamir_multiview_net*.pt'))
            assert len(tmp1) == 1
            logging.info('Loading pamir_multiview_net from ' + tmp1[0])
            data = torch.load(tmp1[0])
        else:
            data = torch.load(model_path)
        if 'pamir_multiview_net' in data:
            self.pamipamir_multiview_netr_net.load_state_dict(data['pamir_multiview_net'])
        else:
            raise IOError('Failed to load pamir_multiview_net model from the specified checkpoint!!')
